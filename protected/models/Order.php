...
class Order extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('order_number, unit_id, status, delivery_date, created_at, created_by', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('order_number', 'length', 'max'=>16),
			array('delivery_provider, delivery_receipt_no, created_by, updated_by', 'length', 'max'=>32),
			array('updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, order_number, unit_id, status, delivery_date, delivery_provider, delivery_receipt_no, created_at, created_by, updated_at, updated_by', 'safe', 'on'=>'search'),
		);
	}
